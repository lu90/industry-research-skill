...
Deep Research Engine turns a user request into a traceable research process: plan, source matrix, multi-step retrieval, evidence classification, cross-checking, contradiction handling, synthesis, and verification. It should improve research discipline without exposing long internal scratch work.

## Execution Flow

1. Research plan: define the mother question, sub-questions, boundary, selected analysis layers, required evidence, and likely source types.
2. Source matrix: map macro, meso, micro, and capital-market claims to preferred sources and evidence levels.
3. First retrieval pass: prioritize official, regulatory, company, industry association, international organization, and credible database sources.
4. Compression pass: summarize findings into facts, opinions, inferences, forecasts, weak evidence, and evidence gaps.
...
8. Cross-verification: for key data and conclusions, try to verify with at least two independent sources or mark the limitation.
9. Contradiction handling: do not force inconsistent sources into one number. Explain definition, geography, period, or source-incentive differences.
...
2. Source matrix or evidence matrix: source type, use, evidence level, and limitation.
...
If network access, database access, filings, or primary sources are unavailable, do not fill gaps with generic claims. Write the evidence gap, the three-round closure attempts already made or the access barrier that stopped the loop early, the reason it remains unresolved, the likely impact on confidence, and the next source to verify.
