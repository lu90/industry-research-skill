...
Use this file before drafting any standard or deep report. It converts the user's natural-language request into an executable research brief so the report route, template, source plan, and depth contract are locked before writing.
...
3. Which template is mandatory, and what opening heading must the report use?
...
| Mandatory template | Exact asset template and required opening heading. |
...
Use `assets/research-prompt-template.md` as the output shape. Fill it with the inferred route, required references, required sections, depth targets, and evidence rules.

The generated prompt must be executable as a report contract. It must name the mandatory template, opening heading, required section checklist, target character range, section-level depth expectations, primary-source-first evidence rules, and rewrite-before-output triggers. For listed-company capital-market questions, it must explicitly require `11.1-11.4`, `4.0 多业务线中观拆分`, `0. 研报前置区`, independent `5.1-5.7`, `12. 多视角压力测试`, and `17. 报告合规自检表`.
...
Rewrite triggers: missing 0, 2.1-2.3, 5.1-5.7, 11.1-11.4, 12, 17, or seven modules compressed into a table.
```
